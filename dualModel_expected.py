...
def main(density, clustering, map_num, gamma, beta, *args):
    ...
    vals = []
    #make the agents repel by default
    if args is None:
        vals.append( -0.1 )
        vals.append( -0.1 )
    else:
    ...
    agents = []
    agents.append( Agent(gamma, beta, data, vals[0], 1) ) # gamma, beta, data, attraction_value, name
    agents.append( Agent(gamma, beta, data, vals[0], 2) )
    for i in range(150):
        for agent in agents:
    ...
    agent_dists = distance.cdist( p1[:,:2], p2[:,:2], 'euclidean' )[:,0]
    
    fig = plt.figure(None, (12,12))
...
if __name__ == '__main__':
    ...
    gamma = 5.5
    beta = 0.125
    main(density, clustering, map_num, gamma, beta, -1.0, -1.0)
